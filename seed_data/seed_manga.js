...
new Promise((resolve) => {
  ...
  async.parallel(
    [
      (callback) => {
        Category.find({}, "_id").exec((err, cate_ids) => {
          callback(null, cate_ids);
        });
      },
      (callback) => {
        Chapter.find({}, "_id").exec((err, chap_ids) => {
          callback(null, chap_ids);
        });
      },
    ],
    (err, results) => {
  ...
  .then((results) => {
    return new Promise((resolve) => {
      ...
      var mangas = [];
      var categories_ = [];
      results[0].forEach((cate) => {
        categories_.push(cate._id);
      });
      ...
      for (i = 0; i < 20; i++) {
        ...
        var categories = categories_
          ...
          .slice(-faker.datatype.number({ min: 2, max: 5 }));
        ...
        mangas.push({
          ...
          releaseDay: faker.random.arrayElement([
            faker.date.past(),
            faker.date.future(),
          ]),
