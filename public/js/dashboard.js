...
function onLoad(data) {
  ...
  initMangaChart(data.topViewManga);
}
...
function initStatusChart(data) {
  ...
  const ctxCategoriesViews = document
    .getElementById("manga_status_chart")
    .getContext("2d");
  //pie chart
...
function initMangaChart(data) {
  ...
  var mangas = [];
  data.forEach((chartItem) => {
    labels.push(chartItem.title);
    mangas.push(chartItem.views);
  });
  const ctxCategoriesViews = document
    .getElementById("manga_created_chart")
    .getContext("2d");
  ...
  const barChart = new Chart(ctxCategoriesViews, {
    ...
    options: {
      plugins: {
        title: {
          display: true,
          text: "Top 10 tryện có lượt xem cao nhất",
          position: "bottom",
        },
        legend: {
          display: false,
